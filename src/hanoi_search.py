...
class HanoiStateSpace:
    ...
    def get_valid_moves(self, state):
        """Generates legal successor states from current configuration."""
        successors = []
        for src in range(3):
            if not state[src]:
                continue
...
def a_star_search(problem):
    """Informed A* Search using heuristic function."""
    start = problem.start_state
    goal = problem.goal_state
    ...
    while pq:
        ...
        for successor, move in problem.get_valid_moves(current_state):
            ...
            if successor not in visited or new_g < visited[successor]:
                ...
                heapq.heappush(pq, (f_score, counter, successor, path + [move], new_g))
...
if __name__ == "__main__":
    ...
    print(f"=== Solving {disks}-Disk Tower of Hanoi State Space ===")
    
    bfs_path, bfs_nodes = breadth_first_search(hanoi)
    print(f"\n[BFS] Moves: {len(bfs_path)} | Expanded States: {bfs_nodes}")
    
    astar_path, astar_nodes = a_star_search(hanoi)
    print(f"[A* ] Moves: {len(astar_path)} | Expanded States: {astar_nodes}\n")

    print("Step-by-step Solution:")
    for step, move in enumerate(astar_path, 1):
